...
根据新增需求，下一阶段优先把整车的一维土料剖面升级为 x-y 高度场及三维交互，
补齐朝向变化下的斗刃扫掠；然后加入铰接转向轨迹控制，实现 A 点铲料、B 点卸料和返回。
第四阶段剩余定位、IMU 融合、地图重定位、四鱼眼/BEV 及联合验收均在这个三维场景完成。
现有直线定位通过结果仅保留作历史基线，不代表新场景已验收。

详细里程碑、范围和验收门槛见 [三维 A→B 开发计划](docs/next_stage_3d_ab.md)。
该调整改变开发先后顺序，不取消下文最终统计复现和连续作业稳定性要求。
