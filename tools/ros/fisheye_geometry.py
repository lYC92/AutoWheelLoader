...
def reproject(rgb,depth,semantic,fov=math.radians(160),output=(640,480),intrinsics=None):
    ...
    finite_corners=np.where(np.isfinite(corners),corners,0.)
    smooth&=(finite_corners.max(axis=0)-finite_corners.min(axis=0))<.05*np.maximum(finite_corners.min(axis=0),.1)
