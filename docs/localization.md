...
2026-09-06，本机三次完整的名义行驶评测均通过 **位置 RMSE ≤ 0.15 m** 门槛。
每轮均执行举升收斗、倒车、制动、前进和停车；独立真值仅供评分。

| 轮次 | 真值行程 | 匹配位姿 | 位置 RMSE | 最大位置误差 | 姿态 RMSE | 里程计频率 |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | 16.839 m | 176 | 0.100 m | 0.195 m | 1.649° | 9.887 Hz |
| 2 | 17.017 m | 196 | 0.116 m | 0.237 m | 1.621° | 10.000 Hz |
| 3 | 17.070 m | 193 | 0.106 m | 0.224 m | 1.583° | 10.000 Hz |

仿真时间延迟 P95 为 54.5–56.0 ms。这里通过的是位置 RMSE 门槛，最大误差仍可能大于 0.15 m。
[三轮原始指标、逐帧轨迹和配置](baselines/2026-09-06/l580_adaptive/README.md)已存档。
三轮使用同一名义丢点种子和同一行驶工况，验证重复启动下的结果，不代表多种子、坡地或动态铲装验收。
模型外观、雷达高度与前处理同时改变，旧新基线不是单一变量对照。
