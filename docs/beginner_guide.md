...
文档版本：2026-09-08。内容以当前项目代码和已保存的测试结果为依据。最新验收进度请看 [三维作业计划](next_stage_3d_ab.md)。

本版包含修复后的 L580 外观和第四阶段定位评测结果；模型尺寸、安装位置等具体数值请以最新模型文件为准。
...
| 定位页面显示位置 | 激光扫描与地图匹配，结合 IMU 估计车辆位置；也保留 KISS-ICP 对照模式 |
| 自动演示完成一套动作 | 程序根据位置、速度、关节和装料量反馈，完成铲取、转弯、卸料及返回 |
...
| 三维料场 | 世界坐标下的车辆、实际土堆网格、规划路线和当前作业步骤 |
...
截至本文版本，真实数据和控制回路自动检查已通过；三维料场新增网格账本检查和 WebSocket 实际传输检查也已通过。登录后的七页布局人工目视验收仍待完成。
...
**2026-09-06 当前结果：**L580 新外观、车顶雷达、自适应地面识别和车体过滤组合下，三次约 17 米的名义行驶试验，位置 RMSE 分别为 **0.100、0.116、0.106 米**，均通过 0.15 米目标。旧版 0.54 米记录保留为历史基线；新旧模型、雷达高度和前处理同时变化，不能只归功于某一个改动。详细指标见 [定位说明](localization.md)。这仅代表当前固定标志物场景通过；IMU 融合、闭环建图、已有地图重定位和动态铲装定位尚未完成。
...
**当前已能演示：**从三维土堆铲料，倒车退出，转弯后卸在另一侧地面，再返回。三维斗刃、随载料变化的质量/质心/惯性已有整车验证；同一场地中两轮 A→B、两轮 B→A 的连续作业也已通过。长时间运行和完整统计验收仍在进行，具体证据见 [三维 A→B 开发计划](next_stage_3d_ab.md)。

| 范围 | 当前实际状态 | 还缺什么 |
| --- | --- | --- |
| 第一阶段：基础环境 | ROS、Gazebo、GPU 和通信基础已跑通 | 持续关注本机运行稳定性 |
| 第二阶段：装载机 | L580 细节外观已接入并修正装配；正式力级控制、基本保护已跑通 | 物理仍为名义模型；厂家几何与实车参数标定，轮胎和液压细化 |
| 第三阶段：砂土 | 三维高度场、不同高度斗刃、动态载荷惯性已接入整车 | 逐颗粒飞散、任意起伏地基和实车材料标定 |
| Foxglove 可视化 | 七页布局已准备；新三维网格与世界坐标车辆的实时传输检查通过 | 登录后对七页布局做目视确认 |
| 第四阶段 M1：传感器效应 | 原始/效应点云逐帧对应，实测均为 10 Hz；IMU 噪声检查通过 | 平移畸变、更多误差来源、目标传感器标定 |
| 第四阶段 M2：定位 | 地图匹配与 IMU 融合已完成一次完整估计位置 A→B，位置 RMSE 8.49 cm | 保存地图重启、失效恢复、多工况与重复统计 |
| 四鱼眼与 BEV | 四相机同步图像、深度、语义、标定、MCAP 和离线俯视图已有移动作业数据验证 | 连续 10 Hz 采集与联合验收收尾 |
| 自主作业 | A/B 状态机、前后车体包络、障碍停车、四轮连续转运已实现 | 静态绕障最后入位、100 轮稳定性和实时性能 |
...
默认 `ab` 场景已使用三维高度场。`soil` 仍保留旧一维场景，便于比较历史结果。相机采用统一的离线等距鱼眼重投影，BEV 是把观察到的表面投影到俯视网格；黑色表示未观察到，绿色表示可行驶地面，红色表示被占用表面。它尚不是训练得到的识别网络。

新增入口（PowerShell，在项目根目录执行）：

```powershell
# 使用激光/IMU 估计位置完成 A/B 作业
.\scripts\windows\run_loader_soil_demo.ps1 -Mode perception -Localization lio_map

# 同时记录四相机，结束后自动输出鱼眼、BEV 与 MCAP
.\scripts\windows\run_loader_soil_demo.ps1 -SurroundCapture

# 同一世界连续四轮：两轮运过去、两轮运回来
.\scripts\windows\run_loader_soil_demo.ps1 -ContinuousCycles 4
```

每次演示的独立结果位于 WSL `/home/lyc/loader_sim_runtime/results/runs/` 下。相机运行目录中的 `surround/bev/` 保存预览图，`surround/mcap/` 保存原始记录，`surround/fisheye_mcap/` 保存可回放的鱼眼数据。连续运行结果在 `continuous/endurance.json`；失败轮次会写明原因。
...
| `[WARN:COPY MODE]`、窗口不出现 | WSLg 图形显示异常 | 有仿真运行时先等待结束；启动器会阻止此时自动重启 WSL |
...
后续开发的完成项、待办和验证数据统一记录在 [下一阶段开发台账](next_stage_3d_ab.md)。
三维料堆已接入整车，可在 Windows 运行：
