...
**2026-09-07 新增目标：**先让料堆在前后、左右方向都能变化，再让装载机从 A 点铲料，沿含转弯的路线到 B 点卸料并返回。接下来的定位、IMU、地图与鱼眼/BEV 工作都放在这个三维场景里验收。具体顺序见 [三维 A→B 开发计划](next_stage_3d_ab.md)。当前只是更新了计划，这些新功能还没有完成。
...
| 第四阶段 M2：定位 | 自适应地面识别与车体过滤已接入；三次名义行驶均通过 0.15 m RMSE 门槛 | IMU 融合、坡地/转向/动态铲装、多工况重复评测 |
