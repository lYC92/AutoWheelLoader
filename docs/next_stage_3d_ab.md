...
当前状态：正在逐项完成本轮六项交付。动态载荷、三维斗刃已完成；包络避障、融合定位和四鱼眼/BEV 已有实现及分项证据，整车联合与长时间验收仍在进行。
第四阶段尚未完成：估计定位闭环、激光/IMU 融合和四鱼眼/BEV 是后续重点。
...
- [ ] 3. 整车包络与避障：已实现前后车体包络、刹停走廊、Hybrid A* 绕行及不可达拒绝；3 项离线测试通过，障碍整车专项待验收。
- [ ] 4. 估计定位闭环：已实现 15 状态误差滤波器、延迟激光更新回放、IMU 杆臂补偿、健康状态制动及恢复重规划，静态偏置/延迟/离群测试通过。
  ...
  已实现地图保存/读取、6 自由度扫描匹配和有界重定位搜索，独立噪声点云恢复及不重叠拒绝测试通过；ROS 集成待完成。
- [ ] 5. 四鱼眼与 BEV：真实 Gazebo 采集首轮通过，5 组各 12 路 RGB/深度/语义严格同时间戳，统一重投影至 640×480、160° 等距鱼眼。
  ...
  待完成移动装卸后土堆一致性、可行驶区独立几何评分及后相机遮挡修正；语义来源是仿真标签，不是训练的识别网络。
- [ ] 6. 联合验收：可视化和 RTF、10 次固定种子统计、100 次连续整车作业及失败归档。
