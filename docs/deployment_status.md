...
M2 的定位精度尚未验收，三维散料整车迁移也仍未完成。网页当前停在 Foxglove 登录页，
六页布局已更新，但登录后的界面目视验收仍待完成。
...
- 第四阶段 M2 已建立固定版本 KISS-ICP 的独立构建、效应点云输入、odom → base_link 输出，
  ...
  当前 16.697 m 行程、185 对位姿的位置 RMSE 为 0.536 m，尚未达到 0.15 m 目标；
  里程计 9.840 Hz。固定高度地面裁剪只适用于当前名义平地场景。
