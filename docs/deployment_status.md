...
当前在第三阶段名义整车/二维土料闭环的基础上，已收尾 Foxglove 信号传输与控制回路，
进入第四阶段：M1 传感器效应链路已回归通过，M2 激光里程计与独立真值评测已接入。
L580 外观已接入并修正装配；M2 在当前固定标志物行驶场景中，三次位置 RMSE 均通过 0.15 m 门槛。
IMU 融合与多工况定位尚未完成；三维散料首版已接入整车并通过三方向挖卸。六页布局已更新，登录后的界面目视验收仍待完成。

下一阶段按新需求调整为：三维土料整车接入 → A→B 转向作业闭环 → 在同一三维环境完成
第四阶段定位/IMU、地图重定位、鱼眼/BEV 与联合验收。详见 [下一阶段计划](next_stage_3d_ab.md)。
有朝向扫掠、C++ 接入和雷达 XY 表面变化检查已通过；A→B 装料、转向、卸料和返回的首轮闭环已通过，综合目标尚未验收。

2026-09-08 补完本轮土料可靠性验证：1000 次同轨重铲、1000 次持续随机挖卸、0.2 m³ 测试容量的整车满斗及域外卸料保护，以及优化后正常 A→B 回归均通过。
长期分片累积已增加保持形状与体积的合并优化；完整第四阶段及 100 次整车连续循环仍未完成。
...
- Foxglove 已增加真实 WebSocket/CDR 验收：physics 为 10 个实际出数通道、14 条字段路径；
  perception 为 14 个实际出数通道、18 条字段路径；原始/效应点云与 IMU 经 TF 连通
  base_link。WebSocket 发出的阀命令可从最终 VehicleCommand 回读，网关超时制动与
  急停锁存/释放也通过。修复了旧演示遗漏激活 joint_state_broadcaster 的问题。
  证据为 `results/foxglove_bridge_{physics,perception}_smoke.txt`，详见 [可观测工作台](observability.md)。
- 第四阶段 M1 已落地：IMU 切换为 Gazebo 内建名义 MEMS 噪声模型（陀螺 σ=0.0017 rad/s、
  ...
  标定扰动参数。9 月 6 日复测为 44 对同时间戳扫描、原始/效应均 10.00 Hz，丢点率
  0.103（目标 0.10）、静止畸变 ≤0.00856 m。修复 NaN 后列时序和行填充处理，正式
  演示自动加载效应配置；可靠订阅修复本机 DDS 大点云整帧丢失。IMU 噪声标准差与配置一致
  （`smoke_test_sensor_effects.sh`，证据见 `results/sensor_effects_smoke.txt`）。
- 第四阶段 M2 已建立固定版本 KISS-ICP 的独立构建、效应点云输入、odom → base_link 输出，
  以及独立 Gazebo 三维真值、时间插值、仅初始位姿对齐的自动误差评测。行驶基线使用
  8 个固定标志物及液压举升后的倒车/前进工况；链路跑通与精度门槛分别记录。
  L580 新外观和车顶外参配合扫描时刻车体过滤、自适应地面识别，三次约 17 m 行驶的
  位置 RMSE 为 0.100 / 0.116 / 0.106 m，均通过 0.15 m 门槛；里程计约 9.89–10 Hz。
  固定高度裁剪保留作历史对照；上述重复结果只覆盖同一名义场景、同一种子。
  当前结果和复现命令见 [定位基线](localization.md)。
...
- [x] 接入 Foxglove 可观测工作台（六页预制布局、手动控制网关）并通过实际消息/控制回路验收
- [ ] 登录 Foxglove 后重新导入六页布局并完成界面目视验收
- [x] 第四阶段 M1：IMU 名义噪声模型、雷达效应通道（丢点/旋转畸变）与传感器安装标定扰动，冒烟测试通过
- [x] 第四阶段 M2 首条基线：KISS-ICP 接入、行驶场景、独立真值与自动误差报告
- [x] 第四阶段 M2 名义行驶精度：自适应地面识别、车体过滤，三轮 RMSE 均不超过 0.15 m
- [ ] 第四阶段 M2 IMU 融合、转向/坡地/动态铲装与多个工况重复评测
...
- 2026-09-06 主机发生 `0xD1 DRIVER_IRQL_NOT_LESS_OR_EQUAL` 蓝屏；事件日志与 minidump
  已定位，但尚未确认具体驱动。恢复后采取串行无界面测试和两个算法线程，未改动驱动。
  详见 [主机故障记录](host_crash_2026-09-06.md)。
